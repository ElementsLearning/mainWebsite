...
export const FormInput: React.FC<FormInputProps> = ({label, value, setValue}) => {
  return (
    <div className='flex flex-1 flex-col gap-2 relative overflow-hidden pt-2'>
      <input className={`bg-transparent flex-1 peer p-1 focus:outline-none border-b-2 ${value !== "" ? "border-black" : "border-neutral-300"}`} value={value} onChange={(e) => setValue(e.target.value)}></input>
